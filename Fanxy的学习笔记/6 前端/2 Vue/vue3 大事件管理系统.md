...
const getInfo = async () => {
  ...
  if (res.data.code === 1) {
    Element.error(res.data.message)
    return
  }
...
**如果自动导入了`ElMessageBox`,记得删掉导入那行代码，本身框架会自动挂载，如果自己导入，会脱出这页，到最底层渲染**
...
空数据直接显示 `no data` 很丑，这里可以使用它内置的`具名插槽`，直接使用**空状态组件。**
...
这里发现各种框架自带的，按钮自带点击后仍然保持 `focus` 是个影响用户体验的响应状态，可以通过添加全局按钮的聚焦效果来改变。 
...
`inline` 可以让表单项展示到一行，如果放不下就继续在下一行进行行填充的操作。
