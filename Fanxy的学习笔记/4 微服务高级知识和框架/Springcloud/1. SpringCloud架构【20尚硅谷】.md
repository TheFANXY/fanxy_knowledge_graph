...
![1微服务技术栈介绍.png](./1. SpringCloud架构.assets/1微服务技术栈介绍.png)
...
![image-20230715160600002](./1. SpringCloud架构.assets/2创建父项目.png)

记得要打开注解驱动设置

![3开启注解驱动](./1. SpringCloud架构.assets/3开启注解驱动.png)
...
![4跳过测试模式.png](./1. SpringCloud架构.assets/4跳过测试模式.png)
...
**这里更推荐使用`@Mapper`是`Java apache`下的注解，而使用`@Repository`更多是注解在实现类，即以前不用`Mybatis`的时候。更简单的：雷神在`Springboot`教过，直接在启动类注解`@MapperScan(baseOacjages = "com.xx.xx")`即可。可能会爆红在注入的时候，因为`IDEA`不知道把对应的`mapper`注入了，无需理会**
...
![5Rundashboard.png](./1. SpringCloud架构.assets/5Rundashboard.png)
...
**但是！通过消费者创建消费者，数据库有主键，但是没有数据，是`NULL`。这是因为跨服务访问，需要在我们的`8001`服务的请求加`@RequstBody`，它可以获取请求体，需要在控制器方法设置一个形参，使用`@RequestBody`进行标识，当前请求的请求体就会为当前注解所标识的形参赋值。`@RequsetBody` 的请求参数是转化为JSON字符串形式，而通过`RestTemplate`传过来的是json数据**
...
<img src="./1. SpringCloud架构.assets/6对Maven进行clean.png" alt="6对Maven进行clean.png" style="zoom: 80%;" />
...
![7Eurekah和Dubbo.png](./1. SpringCloud架构.assets/7Eurekah和Dubbo.png)
...
![8Eureka网页端.png](./1. SpringCloud架构.assets/8Eureka网页端.png)
...
![9注册.png](./1. SpringCloud架构.assets/9注册.png)
...
![10消费者注册.png](./1. SpringCloud架构.assets/10消费者注册.png)

## 2.3. 集群Eureka构建步骤

### 2.3.1. Eureka集群原理说明

![11Eruka集群原理.png](./1. SpringCloud架构.assets/11Eruka集群原理.png)
...
![12服务器集群.png](./1. SpringCloud架构.assets/12服务器集群.png)
...
![13主机名.png](./1. SpringCloud架构.assets/13主机名.png)
...
![14对应关系.png](./1. SpringCloud架构.assets/14对应关系.png)
...
![15自我保护.png](./1. SpringCloud架构.assets/15自我保护.png)
...
![16故障原因1.png](./1. SpringCloud架构.assets/16故障原因1.png)

![17故障原因2.png](./1. SpringCloud架构.assets/17故障原因2.png)

![18故障原因3.png](./1. SpringCloud架构.assets/18故障原因3.png)
...
![19关闭效果.png](./1. SpringCloud架构.assets/19关闭效果.png)
...
配置文件和springcloud没有太大区别，也是服务名称，端口号，以及对应zookeeper的ip等属性配置，这里就使用我之前学了zookeeper配置的三台集群。
...
主启动以后我们使用别的注册中心一般就不用之前专门给Eureka的注解了，直接使用服务发现的注解
...
![20zookeeper.png](./1. SpringCloud架构.assets/20zookeeper.png)

<img src="./1. SpringCloud架构.assets/21jar包.png" alt="21jar包.png" style="zoom: 67%;" />
...
![22zookeeper成功.png](./1. SpringCloud架构.assets/22zookeeper成功.png)

测试是否能通过web端注册：http://localhost:8004/payment/zk

![23zookeeper detial.png](./1. SpringCloud架构.assets/23zookeeper detial.png)
...
![24消费者.png](./1. SpringCloud架构.assets/24消费者.png)
...
![25Consul.png](./1. SpringCloud架构.assets/25Consul.png)

![26特性.png](./1. SpringCloud架构.assets/26特性.png)
...
![30异同.png](./1. SpringCloud架构.assets/30异同.png)
...
<img src="./1. SpringCloud架构.assets/27CAP.png" alt="27CAP.png" style="zoom: 67%;" />

**AP(Eureka)**

![28AP.png](./1. SpringCloud架构.assets/28AP.png)

**CP(Zookeeper/Consul)**

![29CP.png](./1. SpringCloud架构.assets/29CP.png)
...
![31Ribbon概述.png](./1. SpringCloud架构.assets/31Ribbon概述.png)
...
![32架构.png](./1. SpringCloud架构.assets/32架构.png)
...
### 6.2.2. 二说RestTemplate的使用

官网：https://docs.spring.io/spring-framework/docs/5.2.2.RELEASE/javadoc-api/org/springframework/web/client/RestTemplate.html

![33resttamplate.png](./1. SpringCloud架构.assets/33resttamplate.png)

#### 1. getForObject方法/getForEntity方法

![34返回对象.png](./1. SpringCloud架构.assets/34返回对象.png)

#### 2. postForObject/postForEntity

![35post.png](./1. SpringCloud架构.assets/35post.png)
...
![36tree.png](./1. SpringCloud架构.assets/36tree.png)
...
![37warning.png](./1. SpringCloud架构.assets/37warning.png)
...
![38Ribbon负载均衡.png](./1. SpringCloud架构.assets/38Ribbon负载均衡.png)
...
![39Feign.png](./1. SpringCloud架构.assets/39Feign.png)

![40cando.png](./1. SpringCloud架构.assets/40cando.png)

**<font color="bb000">Feign和OpenFeign两者区别</font>**

![41区别.png](./1. SpringCloud架构.assets/41区别.png)
...
**yaml配置文件，这里使用`open-feign`就在配置文件不注册到`eureka`里面作为服务了**
...
<img src="./1. SpringCloud架构.assets/43集成ribbon.png" alt="43集成ribbon.png" style="zoom:67%;" />
...
<img src="./1. SpringCloud架构.assets/42Timeout.png" alt="42Timeout.png" style="zoom:67%;" />
...
![44feign日志.png](./1. SpringCloud架构.assets/44feign日志.png)

# 8. Hystrix断路器【已停止维护】

## 8.1. 概述

![45分布式问题.png](./1. SpringCloud架构.assets/45分布式问题.png)
...
![46概述.png](./1. SpringCloud架构.assets/46概述.png)
...
![47压力测试.png](./1. SpringCloud架构.assets/47压力测试.png)
...
![48兜底方案.png](./1. SpringCloud架构.assets/48兜底方案.png)
...
![49注解.png](./1. SpringCloud架构.assets/49注解.png)
...
**<font color="bb000">必须要加`@Cimponent`</font>**
...
![50介绍.png](./1. SpringCloud架构.assets/50介绍.png)
...
![51机制.png](./1. SpringCloud架构.assets/51机制.png)
...
![52流程图.png](./1. SpringCloud架构.assets/52流程图.png)

断路器在什么情况下开始起作用

![53熔断器.png](./1. SpringCloud架构.assets/53熔断器.png)
...
![54断路器打开后.png](./1. SpringCloud架构.assets/54断路器打开后.png)
...
![55原理图.png](./1. SpringCloud架构.assets/55原理图.png)

![56原理表格.png](./1. SpringCloud架构.assets/56原理表格.png)
...
![57监控.png](./1. SpringCloud架构.assets/57监控.png)
...
![58图.png](./1. SpringCloud架构.assets/58图.png)
...
![59说明.png](./1. SpringCloud架构.assets/59说明.png)

![60说明2.png](./1. SpringCloud架构.assets/60说明2.png)

![61大图.png](./1. SpringCloud架构.assets/61大图.png)
...
![62网关.png](./1. SpringCloud架构.assets/62网关.png)

Gateway是在Spring生态系统之上构建的API网关服务，基于Spring 5，Spring Boot 2和 Project Reactor等技术。
Gateway旨在提供一种简单而有效的方式来对API进行路由，以及提供一些强大的过滤器功能， 例如：熔断、限流、重试等

![63概述.png](./1. SpringCloud架构.assets/63概述.png)

SpringCloud Gateway 使用的Webflux中的reactor-netty响应式编程组件，底层使用了Netty通讯框架。

<img src="./1. SpringCloud架构.assets/64包结构.png" alt="64包结构.png" style="zoom:67%;" />
...
![65架构.png](./1. SpringCloud架构.assets/65架构.png)
...
![66结构.png](./1. SpringCloud架构.assets/66结构.png)

![67缺点.png](./1. SpringCloud架构.assets/67缺点.png)

GateWay模型

![68webflux.png](./1. SpringCloud架构.assets/68webflux.png)
...
![69总体](./1. SpringCloud架构.assets/69总体.png)

## 9.3. 工作流程

![70官网.png](./1. SpringCloud架构.assets/70官网.png)
...
![71路由](./1. SpringCloud架构.assets/71路由.png)
...
![72官网案例.png](./1. SpringCloud架构.assets/72官网案例.png)
...
![73是什么.png](./1. SpringCloud架构.assets/73是什么.png)

Route Predicate Factories 是什么？

![74官网介绍.png](./1. SpringCloud架构.assets/74官网介绍.png)

### 9.6.2. 常用的Route Predicate

#### 1. After Route Predicate

![75datatime.png](./1. SpringCloud架构.assets/75datatime.png)
...
![76before.png](./1. SpringCloud架构.assets/76before.png)
...
![77false.png](./1. SpringCloud架构.assets/77false.png)
...
![80host.png](./1. SpringCloud架构.assets/80host.png)
...
![81get.png](./1. SpringCloud架构.assets/81get.png)
...
![82path.png](./1. SpringCloud架构.assets/82path.png)
...
![83query.png](./1. SpringCloud架构.assets/83query.png)
...
![84过滤器.png](./1. SpringCloud架构.assets/84过滤器.png)
...
- 1. **GatewayFilter**  https://cloud.spring.io/spring-cloud-static/spring-cloud-gateway/2.2.1.RELEASE/reference/html/#the-addrequestparameter-gatewayfilter-factory![85种类.png](./1. SpringCloud架构.assets/85种类.png)
- 2. GlobalFilter

![85种类.png](./1. SpringCloud架构.assets/85种类.png)
...
<img src="./1. SpringCloud架构.assets/86config.png" alt="86config.png" style="zoom:67%;" />

![87概述.png](./1. SpringCloud架构.assets/87概述.png)
...
![88配置.png](./1. SpringCloud架构.assets/88配置.png)
...
![89hosts.png](./1. SpringCloud架构.assets/89hosts-1689851716455-1.png)
...
![90relus.png](./1. SpringCloud架构.assets/90relus.png)
...
![91总结.png](./1. SpringCloud架构.assets/91总结.png)
...
**这里其实也能发现，配置文件这个启动器和3344不同，没有server后缀**
...
![92bootstrap.png](./1. SpringCloud架构.assets/92bootstrap.png)**bootstrap > extension > application**
...
![93说明.png](./1. SpringCloud架构.assets/93说明.png)
...
![94修改.png](./1. SpringCloud架构.assets/94修改.png)

![95dev.png](./1. SpringCloud架构.assets/95dev.png)

**发现3344端口这里立马发生配置更改，但3355没有变化**

![96dev2.0.png](./1. SpringCloud架构.assets/96dev2.0.png)
...
![97bus.png](./1. SpringCloud架构.assets/97bus.png)

![98bus ability.png](./1. SpringCloud架构.assets/98bus ability.png)

![99bus原理.png](./1. SpringCloud架构.assets/99bus原理.png)

## 11.2. RabbitMQ环境配置

  这里我没用windows版，使用的虚拟机的rabbitmq
