...
![1.png](./第17章 其它数据库日志.assets/1.png)
...
![2.png](./第17章 其它数据库日志.assets/2.png)
...
![3.png](./第17章 其它数据库日志.assets/3.png)
...
![4.png](./第17章 其它数据库日志.assets/4.png)
...
![5.png](./第17章 其它数据库日志.assets/5.png)

![6.png](./第17章 其它数据库日志.assets/6.png)

![7.png](./第17章 其它数据库日志.assets/7.png)

#### **4.** **二进制日志(bin log)** 

**<font color="bb000">bin log是MySQL服务端层面的日志，undo log是存储引擎层面InnoDB的日志</font>**

![8.png](./第17章 其它数据库日志.assets/8.png)

![9.png](./第17章 其它数据库日志.assets/9.png)

##### **4.1** **查看默认情况**

![10.png](./第17章 其它数据库日志.assets/10.png)
...
![11.png](./第17章 其它数据库日志.assets/11.png)
...
![12.png](./第17章 其它数据库日志.assets/12.png)
...
![13.png](./第17章 其它数据库日志.assets/13.png)
...
> 注意：使用`mysqlbinlog`命令进行恢复操作时，必须是编号小的先恢复，例如atguigu-bin.000001必须在atguigu-bin.000002之前恢复。
...
![14.png](./第17章 其它数据库日志.assets/14.png)

![15.png](./第17章 其它数据库日志.assets/15.png)

![16.png](./第17章 其它数据库日志.assets/16.png)

#### **5.** **再谈二进制日志(`binlog`)** 

##### **5.1** **写入机制** 

`binlog`的写入时机也非常简单，事务执行过程中，先把日志写到`binlog cache`，事务提交的时候，再把`binlog`cache写到`binlog`文件中。因为一个事务的`binlog`不能被拆开，无论这个事务多大，也要确保一次性写入，所以系统会给每个线程分配一个块内存作为`binlog` cache。

我们可以通过binlog_cache_size参数控制单个线程 binlog cache 大小，如果存储内容超过了这个参数，就要暂存到磁盘(Swap) 。binlog日志刷 盘流程如下:

<img src="./第17章 其它数据库日志.assets/17.png" alt="17.png" style="zoom: 67%;" />
...
<img src="./第17章 其它数据库日志.assets/18.png" alt="18.png" style="zoom:67%;" />

为了安全起见，可以设置为`1`，表示每次提交事务都会执行fsync，就如同**redo log** **刷盘流程**一样。最后还有一种折中方式，可以设置为N(N>1)，表示每次提交事务都write，但累积N个事务后才fsync。

<img src="./第17章 其它数据库日志.assets/19.png" alt="19.png" style="zoom:67%;" />
...
<img src="./第17章 其它数据库日志.assets/20.png" alt="20.png" style="zoom: 80%;" />

在执行更新语句过程，会记录redo log与binlog两块日志，以基本的事务为单位，redo log在事务执行过程中可以不断写入，而binlog只有在提交事务时才写入，所以redo log与binlog的`写入时机`不一样。

为了解决两份日志之间的逻辑一致问题，InnoDB存储引擎使用**两阶段提交**方案。

<img src="./第17章 其它数据库日志.assets/21.png" alt="21.png" style="zoom:80%;" />

![22.png](./第17章 其它数据库日志.assets/22.png)

![23.png](./第17章 其它数据库日志.assets/23.png)

<img src="./第17章 其它数据库日志.assets/24.png" alt="24.png" style="zoom:80%;" />

![25.png](./第17章 其它数据库日志.assets/25.png)

![26.png](./第17章 其它数据库日志.assets/26.png)
